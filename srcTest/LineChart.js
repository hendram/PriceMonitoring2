import React, {useEffect, useRef, ReactElement, useState, RefObject} 
from "react"
...
*/
const useRefDimension = (ref/*: RefObject<HTMLDivElement>*/) => {
     const [dimension, setDimension] = useState({width: NaN, height: NaN});
     useEffect(() => {
     if(ref.current !== null){
       const boundRect = ref.current.getBoundingClientRect();
      const { width, height } = boundRect;
     console.log('widthnya' + width);

        let a;
        let b;
        if(!isNaN(dimension.width)){
               a = dimension.width;
           }
         if(!isNaN(dimension.height)){
          ...
          if(a && b){
      setDimension({ width: a - width, height: b - height})
          }
         else {
...
const LineChart = () => {
let data = [{X: "1.4", y: "3:05:50"}, {x: "1.7", y: "3:10:50"}];
let namatoken ="USDC";
let tokenacuan = "WMATIC";
let priceawal = "1";


const [divlchart, setDivlchart] = useState({svghidorsh: "svghid", 
divwaithidorsh: "divwaitsh", rerender: "unlock"});

const lchartwidhe = useRef/*<HTMLDivElement>*/(null);
const dimension = useRefDimension(lchartwidhe);
console.log('dimesionwidth' + dimension.width);
console.log('dimensionheight' + dimension.height);
...
if(divlchart.rerender !== "lock") {
    let newdivlchart = {divwaithidorsh: "divwaithid", svghidorsh: "svgsh",
rerender: "lock"}
    setDivlchart(newdivlchart);
}
}


return(
<div className="divtoplinechart" >
     <div className={divlchart.divwaithidorsh}> 
Waiting at least two points in graphics

</div>
<div className={divlchart.svghidorsh} ref={lchartwidhe} >
